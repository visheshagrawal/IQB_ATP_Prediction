...
def maindo(string,x,y): #x and y are arrays that we append to 
    ...
    for i in range(len(string)-6):
        x.append(convertStringtoMatrix(string[i:i+7]))
        if ord(string[i+3])>64 and ord(string[i+3])<96:
            y.append(-1)
        else:
            y.append(1)
    return x,y
...
x = [ [ [0 for cc1 in range(7)] for cc2 in range(21)]]
y=[]    
len(df_train.index)
